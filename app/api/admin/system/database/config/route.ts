...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.deleteProfile) {
      if (!body.name) return NextResponse.json({ message: "Profile name required" }, { status: 400 });
    ...
    if (body.saveToProfiles) {
      if (!body.name) return NextResponse.json({ message: "Profile name required" }, { status: 400 });
      if (!body.config) return NextResponse.json({ message: "Profile config required" }, { status: 400 });
    ...
    if (!body.host && !body.databaseUrl) {
      return NextResponse.json({ message: "Provide host + database or databaseUrl" }, { status: 400 });
    }
